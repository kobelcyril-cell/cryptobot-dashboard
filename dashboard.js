...
fetch(`dashboard-data.json?t=${Date.now()}`,{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json()}).then(data=>{if(!hasNewAdaData(data)){data.recent_trades=(data.recent_trades||[]).filter(t=>t.symbol!=="ADA_USD");if(data.strategies?.scalping)data.strategies.scalping={...data.strategies.scalping,name:"ADA · 6-Kerzen-Reversion",return_pct:0,completed_trades:0,status:"Wartet auf Bot-Neustart"}}render(data);renderAdaActivity(data)}).catch(error=>{console.error("Dashboard-Daten konnten nicht geladen werden:",error);$("status").classList.add("offline");$("status").querySelector("span").textContent="DATEN NICHT ERREICHBAR"});
...
fetch(`dashboard-data.json?t=${Date.now()}`,{cache:"no-store"}).then(r=>r.json()).then(data=>{
  ...
  renderStrategyChart("scalp-chart","scalp-chart-tooltip","scalp-chart-value",hasNewAdaData(data)?data.strategy_history?.scalping:[],hasNewAdaData(data)?data.strategy_markers?.scalping||[]:[]);
});
